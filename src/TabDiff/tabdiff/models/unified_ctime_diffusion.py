...
class UnifiedCtimeDiffusion(torch.nn.Module):
    ...
    def mixed_loss(self, x, guidance: torch.Tensor, guidance_mask:torch.Tensor):
        ...
        # Continuous forward diff 连续（数值类型数据）前向扩散过程
        x_num_t = x_num
        if x_num.shape[1] > 0:
            noise = torch.randn_like(x_num)
            #  生成t步噪声数据：x_t^num = x_0^num + σ(t)·ε
            x_num_t = x_num + noise * sigma_num
        
        # Discrete forward diff 分类（离散类型数据）前向扩散过程
        x_cat_t = x_cat
    ...
    def q_xt(self, x, move_chance, strategy='hard'):
        ...
        """
        """
        对输入的原始分类特征 x，按 move_chance 概率执行掩码操作，输出 “加噪后的分类数据”——
          既包含离散的类别索引（xt，用于后续推理采样），
          也包含连续的概率分布（xt_soft，用于训练时梯度传播），适配不同调度模式的需求。
        """
        if strategy == 'hard':
    ...
    def edm_update(
        ...
        has_cat = len(self.num_classes) > 0
        """
        在 TABDIFF 反向去噪的 (t -> t-1) 步骤中，核心逻辑正是 
        “先在当前 t 步基础上小幅加噪得到 (t^+)，再从 (t^+) 反向去噪至
          (t-1)”，这一过程通过 “可控扰动 + 重新去噪” 实现累积误差的修正
        """
