...
class ResidualBlock(nn.Module):
    ...
    def __init__(self, in_length: int, in_channels: int, out_length: int, out_channels: int, time_emb_length: int,
                 num_groups:int=16, dropout: float = 0.1):
        """
        Params:
        ...
        # 第一层线性层
        self.norm1 = nn.GroupNorm(num_groups,in_channels)# nn.GroupNorm(num_groups,in_channels)
        self.act1 = Swish()
        # self.conv1 = nn.Conv1d(in_channels, out_channels, kernel_size=3, padding=1)
        self.linear1 = MLP(in_channels, out_channels) # MultiLinearLayer2d(in_features=in_length,in_channels=in_channels, out_features=out_length,out_channels=out_channels)

        # 第二层线性层
        self.norm2 = nn.GroupNorm(num_groups,out_channels) #nn.GroupNorm(num_groups,out_channels)
        self.act2 = Swish()
        # self.conv2 = nn.Conv1d(out_channels, out_channels, kernel_size=3, padding=1)
        self.linear2 = MLP(out_channels,out_channels) # MultiLinearLayer2d(in_features=out_length,in_channels=out_channels, out_features=out_length,out_channels=out_channels)


        # 当in_c = out_c时,残差连接直接将输入输出相加；
        # 当in_c != out_c时,对输入数据做一次卷积,将其通道数变成和out_c一致,再和输出相加
        if in_length != out_length: # or in_channels != out_channels:
            # self.shortcut = nn.Conv1d(in_channels, out_channels, kernel_size=1)
            self.shortcut = MLP(in_length, out_length) # MultiLinearLayer2d(in_features=out_length,in_channels=in_channels, out_features=out_length,out_channels=out_channels)
        else:
            self.shortcut = nn.Identity()

        # t向量的维度time_channels可能不等于out_c,所以要做一次线性转换
        self.time_emb = nn.Linear(time_emb_length, out_length)# nn.Linear(time_emb_length, out_channels)
        self.time_act = Swish()
...
class AttentionBlock(nn.Module):
    ...
    def __init__(self, n_channels: int, num_groups:int = 16, n_heads: int = 1, d_k: int = None):
        """
        Params:
    ...
    def forward(self, x: torch.Tensor, t: Optional[torch.Tensor] = None):
        ...
        # 将输入数据的shape改为(batch_size, length, n_channels)
        ...
        res = res.permute(0, 2, 1).view(batch_size, channels, length)
        return res
...
class DownBlock(nn.Module):
    ...
    def __init__(self, in_length: int, out_length: int,in_channels: int, out_channels: int,
                  time_emb_length: int, class_dims:int, has_attn: bool):
        super().__init__()
        self.res = ResidualBlock(in_length, in_channels, out_length, out_channels, time_emb_length)
        self.guidance_emb = ClassEmbedding(class_dims)
        if has_attn:
    ...
    def forward(self, x: torch.Tensor, t: torch.Tensor, class_ : torch.Tensor):
        ...
        x += class_embedding
        # x = self.res(x, t, is_feat=True,is_ch=False)
        x = self.res(x, t)
        x = self.attn(x)
        return x
...
class UpBlock(nn.Module):
    def __init__(self, in_length: int, out_length: int,in_channels: int, out_channels: int,
        ...
        self.res = ResidualBlock(in_length+in_length, in_channels + out_channels, out_length, out_channels, time_emb_length)
...
class MiddleBlock(nn.Module):
    def __init__(self, n_length: int, n_channels, time_emb_length: int):
        super().__init__()
        self.res1 = ResidualBlock(n_length, n_channels, n_length, n_channels, time_emb_length)
        self.attn = AttentionBlock(n_channels)
        self.res2 = ResidualBlock(n_length, n_channels, n_length, n_channels, time_emb_length)
...
class Upsample(nn.Module):
    def __init__(self, in_length:int, out_length:int, n_channels:int):
        super().__init__()
        self.trans = MLP(in_length,out_length)# MultiLinearLayer2d(in_length, n_channels, out_length, n_channels)

    def forward(self, x: torch.Tensor, t: torch.Tensor, class_: torch.Tensor = None):
        _ = t,class_
        return self.trans(x)
...
class UNet(nn.Module):
    def __init__(self, unet_config:UnetConfig):
        ...
        * `n_blocks` is the number of `UpDownBlocks` at each resolution
        ...
        n_length: int = unet_config.n_length
        ...
        # self.vector_proj = MLP(feature_length,n_length)

        # Time embedding 层 将时间步t输出为时间嵌入
        ...
        for i in range(n_resolutions):
            ...
            for _ in range(n_blocks):
                down.append(DownBlock(in_length, out_length, in_channels, out_channels
                                      , time_guidance_length, self.num_class, is_attn[i]))
                in_channels = out_channels
            if i < n_resolutions - 1:
                out_length = in_length // 2
                down.append(Downsample(in_length,out_length,in_channels))
        ...
        # Middle block
        self.middle = MiddleBlock(out_length,out_channels,time_guidance_length)
        ...
        for i in reversed(range(n_resolutions)):
            ...
            for _ in range(n_blocks):
                up.append(UpBlock(in_length,out_length,in_channels, out_channels
                                  , time_guidance_length, self.num_class, is_attn[i]))
            ...
            if i > 0:
                out_length = in_length * 2
                up.append(Upsample(in_length,out_length,out_channels))
                in_length = out_length
        ...
        self.norm = nn.BatchNorm1d(in_length)
        self.act = Swish()
        self.final = MLP(in_length, feature_length) #MultiLinearLayer2d(in_length,in_channels,feature_length,1)
